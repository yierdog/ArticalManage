...
function goPage(index) {
    ...
    $.ajax({
        type: "Post",
        url: "../server/ArticleManage.aspx/GetarticallData",
...
function ShowList(list) {
    ...
    for (var i = 0; i < list.length; i++) {
        aStr += '<li><dl><dt class="messdt" id=""></dt><dd>' +
    ...
    $("#articleList_coo").html(aStr);
}


//查询文章热点总数
$(document).ready(function () {
    getartcalArticlecount();
})
function getartcalArticlecount() {
    $.ajax({
        type: "Post", //http通信传参方式
        url: "../server/ArticleManage.aspx/GetarticalArticleListcount", //服务器端资源
